...
public:
	...
	static QVector2D projLatLonToMeter(double longitude, double latitude);
	static QVector2D projMeterToLatLon(double x, double y, const QVector2D &centerLatLon);
	static std::vector<double> getGeoInfo(char * filename);
	static QVector2D getXYCoordSeg(double x, double y);
	static QVector2D getLongLatCoord(double x, double y);
	static QVector2D getXYCoord(double x, double y, const std::vector<double> paras);
